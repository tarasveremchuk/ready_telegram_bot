...
def infoChapter(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    button1 = types.KeyboardButton('✏️ Звернутися до підтримки ✏️')
    button2 = types.KeyboardButton('↩️ Назад до меню')
    markup.row(button1, button2)
    word1 = "  "
...
def my_items(message):
    ...
    if order_numbers:
        ...
        bot.reply_to(message, f"Ви ще не створювали замовлення.")
...
def handle_callback_query(call):
    ...
    if call.data == 'yes':
        if get_price_status(current_order_number) is None:
            update_price_status(current_order_number, "yes")  # Змінити статус на 4

            send_delivery_options(message, owner_id, group_id)
            update_price_status(current_order_number, "no")  # Змінити статус на 3
        else:
        ...
        if get_price_status(current_order_number) is None:
            bot.send_message(message.chat.id,
                             "Наш працівник ознайомиться з вашим рішенням, можливо, запропонує ще якусь пропозицію!")
