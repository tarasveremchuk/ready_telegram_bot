...
def settingsChapter(message):
    ...
    button3 = types.KeyboardButton('↩️ Назад до меню')
    markup.row(button3)
    bot.send_message(message.chat.id, '📸 Натисни "Продати річ" ➡️ ”📷 Відправити фото 📸” та надішли нам потрібні фото своєї речі.\n\n'
                                      '🚀 Наші колеги оперативно розглянуть твою пропозицію та запропонують найкращу можливу ціну.\n\n'
                                      '👀 В розділі "Мої речі" зможеш відстежувати статус своєї пропозиції.\n\n'
                                      ...
                                      '📦 Після відправлення товару, надай номер накладної, номер карти (в випадку якщо ти обрав достаку через систему) натиснувши:  “Мої речі” ➡️ “Відправити номер карти” “Відправити номер накладної”.\n\n'
...
def send_all_photos(message):
    ...
    if len(photo_records) > 0:
        ...
        bot.send_message(message.chat.id, '✅ Ваші фото були успішно завантажені 😌\n\n'
                                          '📍Щоб переглянути статус пропозиції перейди до розділу "Мої речі".\n\n'
                                          '📍Один з наших працівників розгляне вашу пропозицію та запропонує вам найкращу ціну, роблячи це максимально швидко 🚀',
                         reply_markup=markup)
    else:
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
...
def OtsinkaTovaru(message):
    text2 = "‼*Як проходить оцінка товару?  🤔*\n\n" \
            ...
            "(”Продати річ” ➡️ ”Речі які ми купуємо”), в цьому розділі ви зможете побачити речі які ми скуповуємо з найвищим пріоритетом, на той чи інший час." \
            "" \
            ""
...
def my_items(message):
    ...
    if order_numbers:
        ...
        bot.reply_to(message, f"В тебе ще немає жодного замовлення 😔. \n📍 Щоб створити замовлення натисни:      \n➡️ ”👕➡️💵 Продати” ➡️ “📷 Відправити фото 📸”.")
...
‼️ Данні для відправки будуть переслані після того, як натиснеш “✅ Обрати доставку наложним платежем”.
 *Недоліки:*
...
‼️ Данні для відправки будуть переслані після того, як натиснеш “✅ Обрати доставку через систему”.
               """
...
def handle_choose_cod(call):
    ...
    if get_delivery_status(current_order_number) is None:
        ...
        text2 = "‼*Відправ свій товар за нижче вказаною адресою:*\n\n" \
